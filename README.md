...
|---|---:|---|---|
| Lua 5.1 | 38 | Yes | Experimental; public layout, profile, disassembly, reference-operand, direct register-`A`/`B`/`C`, root and nested conditional RK-`B`/`C`, and closure-capture source-bound evidence exists alongside closure identity, query, and machine-interface evidence; exact target promotion remains pending |
