...
use luad_dialect_lua51::{disassemble_proto_lua51, validate_chunk_lua51, Lua51Dialect};

fn exercise_prototype_analysis(proto: &Prototype) {
    let lifted = lift_proto_for_dialect("lua5.1", proto);
    serde_json::to_vec(&lifted).expect("Lua 5.1 lifted facts must serialize");

    let cfg = ControlFlowGraph::build(proto, &lifted);
    serde_json::to_vec(&cfg).expect("Lua 5.1 CFG facts must serialize");
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(chunk) = dialect.decode_chunk(&mut reader_strict) {
        let disasm = disassemble_proto_lua51(&chunk.main_proto);
        serde_json::to_vec(&disasm).expect("Lua 5.1 disassembly facts must serialize");

        let (verdict, diags) = validate_chunk_lua51(&chunk);
        serde_json::to_vec(&(verdict, &diags))
            .expect("Lua 5.1 validation facts must serialize");

        exercise_prototype_analysis(&chunk.main_proto);

        let xrefs = XrefIndex::build(&chunk);
        serde_json::to_vec(&xrefs).expect("Lua 5.1 xref facts must serialize");
    }
});
