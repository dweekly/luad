//! Structural and VM invariant validator for Lua 5.2 bytecode chunks.
...
/// Validate Lua 5.2 chunk invariants.
pub fn validate_chunk_lua52(chunk: &Chunk) -> (Verdict, Vec<Diagnostic>) {
...
fn uses_register_a(op: Opcode52) -> bool {
    !matches!(
        op,
        Opcode52::ExtraArg
            | Opcode52::Jmp
...
fn validate_proto(proto: &Prototype, diags: &mut Vec<Diagnostic>) {
    ...
    for (pc, inst) in proto.instructions.iter().enumerate() {
        ...
        match op {
            Opcode52::LoadKx => {
                let has_extra = if pc + 1 < num_insts {
                    let next_raw = RawInstruction52::decode(proto.instructions[pc + 1].raw_word);
                    next_raw.opcode == Some(Opcode52::ExtraArg)
                } else {
                    false
                };
                if !has_extra {
            ...
            Opcode52::ExtraArg => {
                let preceded_by_kx = if pc > 0 {
                    let prev_raw = RawInstruction52::decode(proto.instructions[pc - 1].raw_word);
                    prev_raw.opcode == Some(Opcode52::LoadKx)
                        || (prev_raw.opcode == Some(Opcode52::SetList) && prev_raw.c == 0)
                } else {
                    false
                };
                if !preceded_by_kx {
        ...
        match op {
            ...
            Opcode52::ForLoop | Opcode52::ForPrep | Opcode52::TForLoop => {
                let dest_pc = (pc as i32 + 1) + raw.sbx;
                if dest_pc < 0 || dest_pc as usize >= num_insts {
                    diags.push(
                        Diagnostic::error(
        ...
        match op {
            ...
            Opcode52::GetTable | Opcode52::SelfOp => {
                if (raw.b as u8) >= proto.maxstacksize {
                    diags.push(
                        Diagnostic::error(
                            "L52-REG-002",
            ...
            Opcode52::ExtraArg if pc > 0 => {
                let prev_raw = RawInstruction52::decode(proto.instructions[pc - 1].raw_word);
                if prev_raw.opcode == Some(Opcode52::LoadKx) && raw.ax as usize >= num_constants {
                    diags.push(
                        Diagnostic::error(
                            "L52-CONST-002",
                            DiagnosticCategory::Instruction,
        ...
        match op {
            Opcode52::GetUpval | Opcode52::SetUpval | Opcode52::GetTabUp => {
            ...
            Opcode52::SetTabUp if raw.a as usize >= num_upvalues => {
                diags.push(
                    Diagnostic::error(
                        "L52-UPVAL-002",
                        DiagnosticCategory::Instruction,
        ...
        if op == Opcode52::Closure && raw.bx as usize >= num_protos {
            diags.push(
                Diagnostic::error(
                    "L52-PROTO-002",
                    DiagnosticCategory::Instruction,
