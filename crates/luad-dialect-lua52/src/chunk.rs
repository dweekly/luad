//! Lossless chunk and prototype undumper for Lua 5.2 bytecode chunks.
...
use crate::header::parse_header_lua52;
use crate::validator::validate_chunk_lua52;

/// Decode an entire Lua 5.2 binary chunk.
pub fn decode_chunk_lua52(reader: &mut SafeReader) -> Result<Chunk, Diagnostic> {
    ...
    let main_proto = load_proto_52(reader, &header, &ProtoPath::root(), None)?;
    ...
    validate_chunk_lua52(&mut chunk);
...
fn load_proto_52(
    ...
    let mut instructions = Vec::with_capacity(sizecode);
    ...
    for idx in 0..sizek {
        ...
        let val = match tag {
            ...
            4 => {
                ...
                s_opt.map(ConstantValue::ShortString).unwrap_or(ConstantValue::Nil)
            }
            other => {
    ...
    let mut protos = Vec::with_capacity(sizep);
    for child_idx in 0..sizep {
        let child_path = path.child(child_idx);
        let mut child_guard = reader.enter_proto(child_idx)?;
        let child_proto = load_proto_52(&mut child_guard, header, &child_path, parent_source)?;
    ...
    let mut line_info = Vec::with_capacity(sizelineinfo * 4);
    let mut abs_line_info = Vec::with_capacity(sizelineinfo);
    ...
    let mut loc_vars = Vec::with_capacity(sizelocvars);
