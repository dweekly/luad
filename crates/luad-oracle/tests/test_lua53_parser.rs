...
use luad_oracle::{compile_and_parse_lua53, compile_source_lua53, find_luac53, verify_truncation_safety_for_dialect};


#[test]
fn test_all_fixtures_lua53_parsing_and_truncation() {
    if find_luac53().is_none() {
        eprintln!("Skipping test: Lua 5.3 compiler not found on system");
        return;
    }
    ...
    for fixture_path in fixture_files {
        ...
        let raw_debug = compile_source_lua53(&source, false).expect("Compilation must succeed");
        let chunk_debug = compile_and_parse_lua53(&source, false).expect("Parsing must succeed");
        assert_eq!(chunk_debug.dialect, "lua5.3");
        ...
        let raw_stripped = compile_source_lua53(&source, true).expect("Stripped compilation must succeed");
        let chunk_stripped = compile_and_parse_lua53(&source, true).expect("Parsing stripped chunk must succeed");
        assert_eq!(chunk_stripped.dialect, "lua5.3");
