...
pub fn lift_proto_lua53(proto: &Prototype) -> Vec<SemanticInstruction> {
    ...
    for (pc, inst) in proto.instructions.iter().enumerate() {
        ...
        let next_word = proto.instructions.get(pc + 1).map(|i| RawInstruction53::decode(i.raw_word));
        let prev_word = if pc > 0 {
            proto.instructions.get(pc - 1).map(|i| RawInstruction53::decode(i.raw_word))
        } else {
            None
        };
...
fn lift_instruction_53(
    ...
    match op {
        ...
        Opcode53::Add | Opcode53::Sub | Opcode53::Mul | Opcode53::Mod | Opcode53::Pow
        | Opcode53::Div | Opcode53::IDiv | Opcode53::BAnd | Opcode53::BOr | Opcode53::BXor
        | Opcode53::Shl | Opcode53::Shr => {
            operands.push(TypedOperand::Register { index: raw.a });
            parse_rk(raw.is_b_k(), raw.b_index_k(), &mut reads, &mut operands);
            parse_rk(raw.is_c_k(), raw.c_index_k(), &mut reads, &mut operands);
            writes.push(EffectTarget::Register { index: raw.a });
