...
Status: the qualification program a 1.0 would require. Not the current plan of record.

Fresh as of: 2026-09-16.

`luad` ships as an experimental 0.x tool today; [ROADMAP.md](../ROADMAP.md) holds the
stack-ranked next steps and is the plan of record. This document is kept because the bar
it describes is the right bar for a 1.0 that a researcher may rely on: exact promoted
targets, pinned compiler authorities, hostile-input bounds, and a frozen machine
contract. Adopt it when the tool is worth that promise, not before.

Product requirements live in [PRD.md](../PRD.md). Exact implementation and acceptance
commands belong in [the active sprint](NEXT-SPRINT.md). This document orders future
obligations; it does not promote a target or authorize product implementation.
...
canonical [release boundary](RELEASING.md#frozen-version-1-boundary):
...
at a time, following [the contract lifecycle](DEVELOPMENT-WORKFLOW.md#12-preservation-documentation-and-escalation).
...
Consult the [prior-art survey](PRIOR-ART-AND-CORPORA.md), verify its dated claims,
